...
Today<-as.Date("2025-07-16",format("%Y-%m-%d"))
Yesterday<-Today-1
Current.Year<-2025
...
Comps%>%
  mutate(Fishery=Fishery.Project, "Composition before July 20th"=Composition*.5,"Composition after July 20th"= Composition)%>%
  select(Fishery, Method, `Composition before July 20th`,`Composition after July 20th`)%>%
  flextable()%>%
  theme_apa()%>%
...
CommTR.0<-TRUnallocated%>%filter(Fate%in%"Commercial Harvest")%>%
  ...
  mutate(Condition=ifelse(Date<as.Date("2025-07-20",format="%Y-%m-%d") ,"yes","no"))%>%
...
Test.2.0%>%left_join(Test.1.0%>%filter(Date%in%Yesterday)%>%select(Stock,Year,Projection))%>%
  ...
  select(Year,MAAPE,Timing,Projection,Weight,Weighted.Projection,Total)%>%
  flextable()%>%
  theme_apa()%>%
  ...
  colformat_num(j=c(4,6,7), digits = 0)%>%
  font(fontname="serif",part="all")%>%
  set_header_labels(Timing="Run timing",Projection="Model projection",Weight="Model weight",Weighted.Projection="Weighted projection")%>%
  merge_v(j=7)%>%
  valign(j=7, valign = "bottom")%>%
  bold(j=7)%>%
  fix_border_issues()%>%
  autofit()
