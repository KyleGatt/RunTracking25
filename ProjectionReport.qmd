...
Today<-as.Date("2025-07-17",format("%Y-%m-%d"))
Yesterday<-Today-1
Current.Year<-2025
...
Test.2.0%>%left_join(Test.1.0%>%filter(Date%in%Yesterday)%>%select(Stock,Year,Projection))%>%
  ...
  mk_par(j = 4, part = "body",value = as_paragraph(as_chunk(Timing, formatter = fmt_pct)))%>%
...
(Test.2.0%>%left_join(Sonar %>%group_by(Year) %>%mutate(Timing = cumsum(Daily.Count / sum(Daily.Count))) %>%filter(Timing >= 0.5) %>%slice_head(n = 1) %>%ungroup() %>%select(Year, Date) %>%arrange(Year) %>%mutate(DateNum = as.numeric(Date),RollingMeanDate = as.Date(rollmean(DateNum, k = 10, align = "right", fill = NA),origin = "1970-01-01"))%>%filter(!is.na(RollingMeanDate))%>%mutate(Timing = round(as.numeric(Date - RollingMeanDate), digits = 0))%>%select(Year,Timing))%>%mutate(Time=mean(Timing), TimingText = case_when(Time < 0 ~ paste0(abs(Time), " days", " early"),Time > 0 ~ paste0(Time, " days", " late "),TRUE       ~ "on time")))$TimingText[1] 
